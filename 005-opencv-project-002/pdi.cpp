...
void PDI::extracaoCaracteristicasBIC()
{
    // 1. Requantização para 64 cores
    Mat entrada = imagemRGB.clone();
    Mat saida = imagemCINZA.clone();
    int const cores = 64;
    PDI::_reducaoCores(entrada, saida);

    // 2. Obtém pixels de borda e internas
    vector<Point> bordas;
    vector<Point> interiores;
    for(int x=0;x<saida.rows;x++)
    {
        for(int y=0;y<saida.cols;y++)
        {
            ...
            // Vizinhança esquerda
            if(x-1 >= 0) {
                if (saida.at<uchar>(x-1,y) != saida.at<uchar>(x,y)) borda = true;
            } else {
                if (saida.at<uchar>(x+1,y) != saida.at<uchar>(x,y)) borda = true;
            }
            // Vizinhança direita
            ...
            // Vizinhança cima
            if(y-1 >= 0) {
                if (saida.at<uchar>(x,y-1) != saida.at<uchar>(x,y)) borda = true;
            } else {
                if (saida.at<uchar>(x,y+1) != saida.at<uchar>(x,y)) borda = true;
            }
            // Vizinhança baixa
    ...
    int histBordas[cores];
    int histInteriores[cores];
    ...
    int nivel = 0;
    vector<Point>::iterator bordasIterator;
    for(unsigned int i=0;i<bordas.size();i++) {
        nivel = (int)saida.at<uchar>(bordas[i].x, bordas[i].y);
        histBordas[nivel] += 1;
    }
    vector<Point>::iterator interioresIterator;
    for(unsigned int i=0;i<bordas.size();i++) {
        Point interior = interiores[i];
        nivel = (int)saida.at<uchar>(interior.x, interior.y);
        histInteriores[nivel] += 1;
    }

    // 4. Concatena histogramas obtidos
    int histFinal[cores*2];
    copy(histBordas, histBordas + cores, histFinal);
    copy(histInteriores, histInteriores + cores, histFinal + cores);

    // 5. Extração das características em um arquivo txt
    String filename = "algoritmo_bic_imagem_" + PDI::_getFilename(imageLoadedName) + ".txt";
    ofstream outputFile (filename);
    if (outputFile.is_open()) {
        for(int i=0;i<cores*2;i++) {
            // if(i == (cores*2)-1) outputFile << i;
            outputFile << i << ",";
        }
        outputFile << "\n";
        for(int i=0;i<cores*2;i++) {
            // if(i == (cores*2)-1) outputFile << histFinal[i];
            outputFile << histFinal[i] << ",";
        }
        outputFile.close();
...
void PDI::extracaoCaracteristicasCadeia()
{
    int const limiar = 100;
    Mat saidaCINZA = imagemCINZA.clone();
    Mat cannySaida;
    vector<vector<Point>> bordas;
    vector<Vec4i> hierarquia;
    // Detecção de borda usando Canny
    Canny(saidaCINZA, cannySaida, limiar, limiar*2, 3);
    ...
    int const cadeiaOrientacoes = 8;
    int histCadeia[cadeiaOrientacoes];
    for(unsigned int i=0;i<cadeiaOrientacoes;i++) {
        histCadeia[i] = 0;
    }
    for(unsigned int i=0;i<cadeiaBorda.size();i++) {
        histCadeia[cadeiaBorda[i]] += 1;
    }

    // Extração de txt
    String filename = "algoritmo_cadeia_imagem_" + PDI::_getFilename(imageLoadedName) + ".txt";
    ofstream outputFile (filename);
    if (outputFile.is_open()) {
        for(int i=0;i<cadeiaOrientacoes;i++) {
            // if(i == cadeiaOrientacoes-1) outputFile << i;
            outputFile << i << ",";
        }
        outputFile << "\n";
        for(int i=0;i<cadeiaOrientacoes;i++) {
            // if(i == cadeiaOrientacoes-1) outputFile << histCadeia[i];
            outputFile << histCadeia[i] << ",";
        }
        outputFile.close();
        QMessageBox *msgBox = new QMessageBox;
        msgBox->setWindowTitle("Sucesso!");
...
void PDI::_reducaoCores(Mat &image, Mat &outputImage)
{

...
//    uchar pixel=0;
//    //criar imagem em branco para o histrograma
//    int altura=L.rows; int largura=L.cols;
//    Mat imagemSaida = imagemCINZA.clone();
//    for(int x=0;x<L.rows;x++)
//    {
//        for(int y=0;y<L.cols;y++)
//        {
//            pixel = L.at<uchar>(x,y);
//            if(pixel > limiar) imagemSaida.at<uchar>(x,y)= 255;
//            //else imagemSaida.at<uchar>(x,y)= 0;
//        }
//    }
    Mat saida;
    //L.copyTo(saida);
...
void PDI::detectorHoughCircle()
{
    Mat Cinza = imagemCINZA.clone();
    Mat saidaBlur;
    blur(Cinza,saidaBlur,Size(5,5));
    vector<Vec3f> circles;
    HoughCircles(saidaBlur, circles, CV_HOUGH_GRADIENT, 1, 50, 200, 10, 0, 0 );
    ...
    /// Draw the circles detected
      for( size_t i = 0; i < circles.size(); i++ )
      {
          ...
          // circle center
          circle( imagemRGB, center, 3, Scalar(0,255,0), -1, 8, 0 );
          // circle outline
          circle( imagemRGB, center, radius, Scalar(0,0,255), 3, 8, 0 );
       }
      imshow("Hough Circles", imagemRGB);
}

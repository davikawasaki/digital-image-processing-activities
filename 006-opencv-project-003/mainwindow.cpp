...
{
    pdi->extracaoCaracteristicasBIC(pdi->imagemRGB.clone(), pdi->imagemCINZA.clone(), pdi->imageLoadedName);
}

void MainWindow::on_actionForma_triggered()
{
    pdi->extracaoCaracteristicasCadeia(pdi->imagemCINZA.clone(), pdi->imageLoadedName);
}

void MainWindow::on_actionExtra_o_M_ltipla_triggered()
{
    //abre uma caixa de dialogo para selecao de múltiplas imagens
    QStringList fileList = QFileDialog::getOpenFileNames(
        this, "Abrir", "/home",
        tr("JPEG (*.jpg *.jpeg);;PNG (*.png);;BMP (*.bmp)")
    );
    pdi->extractMultipleFeatures(fileList);
}
